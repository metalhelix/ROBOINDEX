...
contig_list=[]
contig_length_list=[]
contig_with_geneMolde = [str(v) for v in df_refflat_txt[2].to_list()]
for record in SeqIO.parse(fna_file,"fasta"):
    if str(record.id) in contig_with_geneMolde:
        contig_list.append(record.id)
        contig_length_list.append(len(record.seq))
genome_length=sum(contig_length_list) # The total genome length need to changed to the total length of the contigs that have gene models
